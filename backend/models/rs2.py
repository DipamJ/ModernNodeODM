...
class RSImage(object):
    ...
    def clip_by_polygon(self, poly):
        ...
        Input "poly" should be polygon from layer (OGR Layer)
        """
        ...
        #maxx and maxy were equal to self.ncol and self.nrow for the
        # Amarillo Wheat project and they were making the code unable to run.
        #I changed the conditional to run when they are the same value

        # Now check if polygon is inside the image
        #if (minx < 0) or (maxx > self.ncol - 1):
        if (minx < 0) or (maxx - 1 > self.ncol):
    ...
    def clip_by_polygon_geopandas(self, boundary_path, feature_index=0):
        """
        Read a GeoJSON or shapefile using geopandas, extract the polygon
        of the specified feature, convert its coordinates to pixel space,
        and clip the RSImage accordingly.
        
        Parameters:
        rs_img: an instance of RSImage with attributes: ds, img, ncol, nrow, geotransform.
        boundary_path: path to the GeoJSON or shapefile.
        feature_index: which feature (polygon) to use.
        
        Returns:
        clipped_img_masked: The image region clipped by the polygon.
        """
        # Read the boundary file with geopandas.
        gdf = gpd.read_file(boundary_path)
        # sys.stdout.write("Original boundary CRS: " + str(gdf.crs) + "\n")
        
        # 2. Reproject the boundary to the image’s CRS.
        target_crs = "EPSG:32614"  # Change if your raster has a different CRS.
        if gdf.crs != target_crs:
            gdf = gdf.to_crs(target_crs)
            # sys.stdout.write("Reprojected boundary to " + target_crs + "\n")
        
        # Choose the feature by index
        geojson_dict = json.loads(gdf.to_json())
        selected_feature = geojson_dict['features'][feature_index]
        
        # Check the geometry type; for this example, we handle Polygon or MultiPolygon
        geom_type = selected_feature['geometry']['type']
        if geom_type == 'Polygon':
            raw_coordinates = selected_feature['geometry']['coordinates']
            # raw_coordinates is a list of rings; we use the first (exterior) ring.
            exterior_coords = raw_coordinates[0]
        elif geom_type == 'MultiPolygon':
            # Use the first polygon's exterior ring
            exterior_coords = selected_feature['geometry']['coordinates'][0][0]
        else:
            raise ValueError(f"Feature {feature_index} is not a Polygon or MultiPolygon.")

        # Clean coordinates: remove any z values if present
        cleaned_coords = [[pt[0], pt[1]] for pt in exterior_coords]
        
        # Convert each world coordinate to pixel coordinates using the image's geotransform.
        geoTrans = self.ds.GetGeoTransform()
        pixels = [world2Pixel(geoTrans, x, y) for (x, y) in cleaned_coords]
        
        # Determine bounding box of these pixel coordinates
        pixels_np = np.array(pixels)
        minx = int(pixels_np[:, 0].min())
        maxx = int(pixels_np[:, 0].max())
        miny = int(pixels_np[:, 1].min())
        maxy = int(pixels_np[:, 1].max())
        
        # Optional: check if the bounding box lies within the image dimensions
        if minx < 0 or miny < 0 or maxx > self.ncol or maxy > self.nrow:
            sys.stdout.write("Warning: The polygon is partially or completely outside the image bounds.")
        
        # Extract the sub-image using the bounding box.
        clipped_img = self.img[:, miny:maxy, minx:maxx]
        
        # Adjust the pixel coordinates relative to the sub-image.
        pixels_adjusted = [(px - minx, py - miny) for (px, py) in pixels]
        
        # Create a binary mask with PIL. The mask size must match the sub-image (width, height).
        mask_img = Image.new("L", (clipped_img.shape[2], clipped_img.shape[1]), 1)  # default value 1 (outside)
        draw = ImageDraw.Draw(mask_img)
        # Draw the polygon: pixels inside the polygon will be set to 0.
        draw.polygon(pixels_adjusted, outline=0, fill=0)
        mask = np.array(mask_img)
        
        # # Convert the mask to a numpy array.
        # mask = gdalnumeric.fromstring(mask_img.tobytes(), 'b')
        # mask.shape = (clipped_img.shape[1], clipped_img.shape[2])
        
        # Apply the mask to each band of the sub-image.
        # Here, we set pixels outside the polygon (mask == 1) to 0.
        clipped_img_masked = np.empty_like(clipped_img)
        for b in range(clipped_img.shape[0]):
            clipped_img_masked[b, :, :] = np.where(mask == 0, clipped_img[b, :, :], 0)
...
class ImageArray():
    ...
    def tofile_utm(self, out_fn):
        ...
        outds.SetGeoTransform([self.left, self.spatial_resolution, 0, self.up, 0, -self.spatial_resolution])
    ...
    def tofile_with_specification(self, out_fn, geo_transform, projection_ref):
        ...
        # Determine data type
        if self.data.dtype == np.int16:
            outds = driver.Create(out_fn, self.data.shape[2], self.data.shape[1], nob, gdal.GDT_Int16)
        elif self.data.dtype == np.int32:
            outds = driver.Create(out_fn, self.data.shape[2], self.data.shape[1], nob, gdal.GDT_Int32)
        elif self.data.dtype == np.float32:
            outds = driver.Create(out_fn, self.data.shape[2], self.data.shape[1], nob, gdal.GDT_Float32)
        elif self.data.dtype == np.float64:
            outds = driver.Create(out_fn, self.data.shape[2], self.data.shape[1], nob, gdal.GDT_Float64)
    ...
    def tofile_as_other_image(self, out_fn, other_image):
        ...
        # Determine data type
        if self.data.dtype == np.uint8:
            outds = driver.Create(out_fn, other_image.ds.RasterXSize, other_image.ds.RasterYSize, nob, gdal.GDT_Byte)
        if self.data.dtype == np.int16:
            outds = driver.Create(out_fn, other_image.ds.RasterXSize, other_image.ds.RasterYSize, nob, gdal.GDT_Int16)
        elif self.data.dtype == np.int32:
            outds = driver.Create(out_fn, other_image.ds.RasterXSize, other_image.ds.RasterYSize, nob, gdal.GDT_Int32)
        elif self.data.dtype == np.float32:
            outds = driver.Create(out_fn, other_image.ds.RasterXSize, other_image.ds.RasterYSize, nob, gdal.GDT_Float32)
        elif self.data.dtype == np.float64:
            outds = driver.Create(out_fn, other_image.ds.RasterXSize, other_image.ds.RasterYSize, nob, gdal.GDT_Float64)
